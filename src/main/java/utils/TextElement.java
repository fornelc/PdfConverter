...
public class TextElement extends Rectangle implements HasText {
    ...
    public static List<TextChunk> mergeWords(List<TextElement> textElements, List<Ruling> verticalRulings) {
        ...
        for (TextElement chr : copyOfTextElements) {
            ...
            if (chr.getText().equals(" ") && Functions.feq(prevChar.getLeft(), chr.getLeft()) && Functions.feq(prevChar.getTop(), chr.getTop())) {
                continue;
            }

            // Resets the average character width when we see a change in font
            // or a change in the font size
            if ((chr.getFont() != prevChar.getFont()) || !Functions.feq(chr.getFontSize(), prevChar.getFontSize())) {
            ...
            // new line?
            sameLine = true;
            if (!Functions.overlap(chr.getBottom(), chr.height, maxYForLine, maxHeightForLine)) {
                endOfLastTextX = -1;
                expectedStartOfNextWordX = -java.lang.Float.MAX_VALUE;
