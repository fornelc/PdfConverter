...
public class Ruling extends Line2D.Float {
    ...
    public void normalize() {
        ...
        if (Functions.within(angle, 0, 1) || Functions.within(angle, 180, 1)) { // almost horizontal
            this.setLine(this.x1, this.y1, this.x2, this.y1);
        }
        else if (Functions.within(angle, 90, 1) || Functions.within(angle, 270, 1)) { // almost vertical
...
//        }
    ...
    public boolean vertical() {
        return this.length() > 0 && Functions.feq(this.x1, this.x2); //diff < ORIENTATION_CHECK_THRESHOLD;
    }
    
    public boolean horizontal() {
        return this.length() > 0 && Functions.feq(this.y1, this.y2); //diff < ORIENTATION_CHECK_THRESHOLD;
    }
    ...
    public static Map<Point2D, Ruling[]> findIntersections(List<Ruling> horizontals, List<Ruling> verticals) {
        ...
        Collections.sort(sos, new Comparator<SortObject>() {
            ...
            public int compare(SortObject a, SortObject b) {
                int rv;
                if (Functions.feq(a.position, b.position)) {
    ...
    public static List<Ruling> collapseOrientedRulings(List<Ruling> lines, int expandAmount) {
        ...
        for (Ruling next_line : lines) {
            ...
            if (last != null && Functions.feq(next_line.getPosition(), last.getPosition()) && last.nearlyIntersects(next_line, expandAmount)) {
